...
const isCollapsed = ref(localStorage.getItem('purchaseChartsCollapsed') === '1')

function toggleCollapsed() {
  isCollapsed.value = !isCollapsed.value
  localStorage.setItem('purchaseChartsCollapsed', isCollapsed.value ? '1' : '0')
}
...
const categoryOptions = computed(() => {
  ...
  for (const purchase of purchases.value) values.add(purchase?.category?.trim() || 'Sans catégorie')
...
const annualSummary = computed(() => {
  ...
  for (const purchase of purchases.value) {
    ...
    const category = purchase?.category?.trim() || 'Sans catégorie'
    if (selectedCategory.value !== 'all' && category !== selectedCategory.value) continue
...
const purchaseAggregates = computed(() => {
  ...
  const sortedCategoryEntries = Array.from(categoryMap.entries()).sort((a, b) => b[1] - a[1])
  const topCategories = sortedCategoryEntries.slice(0, MAX_VISIBLE_CATEGORIES)
  const otherCategories = sortedCategoryEntries.slice(MAX_VISIBLE_CATEGORIES)
  const otherTotal = otherCategories.reduce((sum, [, amount]) => sum + amount, 0)
  const finalCategoryEntries = otherTotal > 0 ? [...topCategories, ['Autres', otherTotal]] : topCategories

  return {
    timelineValues: timelineEntries.map(([, total]) => total),
    categoryLabels: finalCategoryEntries.map(([category]) => category),
    categoryValues: finalCategoryEntries.map(([, total]) => total),
  }
})
...
const selectedMonthDetails = computed(() => {
  ...
  const topCategories = Array.from(breakdown.entries())
    .sort((a, b) => b[1] - a[1])
    .slice(0, 4)
...
const topCategoriesSummary = computed(() => {
  const total = purchaseAggregates.value.categoryValues.reduce((sum, value) => sum + value, 0)

  return purchaseAggregates.value.categoryLabels.map((label, index) => {
    const value = purchaseAggregates.value.categoryValues[index] || 0
...
const annualPulse = computed(() => {
  ...
  return `${delta > 0 ? 'Dépenses en hausse' : delta < 0 ? 'Dépenses en baisse' : 'Rythme stable'} · ${formatPercent(delta)}`
})

const dashboardHighlights = computed(() => {
  const totalVisible = purchaseAggregates.value.timelineValues.reduce((sum, value) => sum + value, 0)
  const topCategory = topCategoriesSummary.value[0]
  const periodsCount = purchaseAggregates.value.timelineValues.length
  ...
  return [
    ...
    },
    {
      ...
      value: topCategory ? `${topCategory.label} · ${topCategory.share.toFixed(0)}%` : 'Aucune',
  ...
  <section class="purchase-dashboard-fintech">
    <article class="fin-hero panel fin-shell">
      <div class="fin-hero-grid">
        <div class="fin-hero-copy">
          ...
          <h2>Comprendre où part votre argent, mois après mois</h2>
          <p class="fin-hero-text">
            Filtrez par période ou catégorie, comparez les mois entre eux et repérez rapidement
            les postes de dépense qui pèsent le plus dans votre budget.
          </p>

          <div class="fin-hero-chips">
            <span class="fin-chip">Suivi par catégorie</span>
            <span class="fin-chip">Comparaison mensuelle</span>
            <span class="fin-chip">Recherche instantanée</span>
          </div>
        </div>
    ...
    <section class="panel fin-shell fin-controls">
      ...
      <div class="fin-controls-grid">
        ...
        <label class="fin-field">
          <span class="fin-field-label">Catégorie</span>
          <select v-model="selectedCategory" class="fin-input">
        ...
        <label class="fin-field fin-field-search">
          ...
          <input
            ...
            type="text"
            class="fin-input"
            placeholder="Commerçant, catégorie, note..."
          />
        </label>
      </div>
    </section>
    ...
    <article class="panel fin-shell fin-list-card">
      <div class="fin-section-head compact-head">
        <div>
          ...
          <h3>Catégories clés</h3>
        </div>
      </div>

      <div v-if="topCategoriesSummary.length" class="fin-source-list">
        <div
          v-for="category in topCategoriesSummary"
          :key="category.label"
          class="fin-source-row"
        >
          <div class="fin-source-meta">
            <span class="fin-source-dot" :style="{ backgroundColor: category.color }"></span>
    ...
    <section class="fin-bottom-grid">
      <article class="panel fin-shell fin-summary-card">
        ...
        <div v-else class="fin-empty-state fin-empty-state-small">
          <p>Aucun achat trouvé sur cet exercice.</p>
        </div>
      </article>
        ...
        <Transition name="fin-fade-up">
          <div v-if="selectedMonthDetails" class="fin-focus-panel">
            ...
            <div class="fin-focus-meta">
              <span>{{ selectedMonthDetails.entryCount }} achats</span>
              <span>{{ Number.isFinite(selectedMonthDetails.variation) ? formatPercent(selectedMonthDetails.variation) : 'Pas de comparaison' }}</span>
            </div>

            <div v-if="selectedMonthDetails.topCategories.length" class="fin-focus-list">
              <div
                v-for="category in selectedMonthDetails.topCategories"
                :key="category.label"
                class="fin-focus-row"
              >
...
:root[data-theme='light'] .purchase-dashboard-fintech {
