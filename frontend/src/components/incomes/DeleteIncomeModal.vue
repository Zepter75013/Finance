...
function confirmDelete() {
  if (!props.income || props.isDeleting) {
    return
  }

  emit('confirm', props.income)
}
</script>

<template>
  <div
    v-if="modelValue && income"
    class="modal-overlay"
  ...
  >
    <section
      ...
      aria-describedby="delete-income-description"
    >
      <div class="modal-header">
        <div class="delete-modal-heading">
          ...
          <div>
            ...
            <h2 id="delete-income-title">Supprimer ce revenu ?</h2>
          </div>
        </div>
      </div>

      <p id="delete-income-description" class="delete-modal-text">
        Tu es sur le point de supprimer le revenu
        <strong>{{ income.source || 'sélectionné' }}</strong>.
        Cette action est irréversible.
      </p>

      <p class="delete-modal-note">
        Vérifie que ce revenu ne fait pas partie d’un import ou d’un suivi en
      ...
      <div class="modal-actions">
        ...
        <button
          ...
          @click="confirmDelete"
        >
          {{ isDeleting ? 'Suppression...' : 'Supprimer le revenu' }}
        </button>
      </div>
    </section>
  </div>
